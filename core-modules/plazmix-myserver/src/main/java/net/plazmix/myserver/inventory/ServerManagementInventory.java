package net.plazmix.myserver.inventory;
...
public class ServerManagementInventory extends CorePaginatedInventory {
    ...
    public ServerManagementInventory(@NonNull PlayerMyServer playerMyServer) {
    ...
    public void drawInventory(CorePlayer player) {
        ...
        for (CorePlayer onlinePlayer : playerMyServer.getCoreServer().getOnlinePlayers()) {
            ...
            itemBuilder.addLore("");
            itemBuilder.addLore("§7Игровой уровень: §b" + onlinePlayer.getLevel());
            itemBuilder.addLore("");
            itemBuilder.addLore("§7Версия клиента: §f" + onlinePlayer.getMinecraftVersionName());
            itemBuilder.addLore("§7Игровой язык: §f" + onlinePlayer.getLanguageType().getDisplayName());
            itemBuilder.addLore("");
            ...
            addItemToMarkup(itemBuilder.build(), (inventory, event) -> {
                ...
                if (onlinePlayer.getName().equalsIgnoreCase(player.getName())) {
                    player.sendMessage("§d§lPlazmix §8:: §cОшибка, Вы не можете кикинуть самого себя!");
                    return;
                }

                onlinePlayer.connectToAnyServer(playerMyServer.getServerType().getCategory().getLobbyPrefix());
                onlinePlayer.sendMessage("§d§lPlazmix §8:: §cВы были кикнуты администратором сервера " + playerMyServer.getServerName());
        ...
        addItem(11, ItemBuilder.newBuilder(Material.SKULL_ITEM)
                        ...
                        .build(),
                (player1, event) -> {
                    player.sendMessage("§d§lMyServer §8:: §fНа сервер была отправлена команда запуска игровой арены");
        ...
        addItem(12, ItemBuilder.newBuilder(Material.SKULL_ITEM)
                ...
                (player1, event) -> {
                    ...
                    if (!playerMyServer.isLeader(player)) {
                        player.sendMessage("§d§lPlazmix §8:: §cОшибка, у Вас недостаточно прав для выполнения данного процесса!");
                        return;
                    }
        ...
        addItem(5, ItemBuilder.newBuilder(Material.SIGN)
                ...
                .addLore("")
                .addLore("§8Статистика:")
                .addLore(" §7Версия ядра: §a" + playerMyServer.getCoreServer().getMinecraftVersionName())
        ...
        addItem(17, ItemBuilder.newBuilder(Material.SKULL_ITEM)
                ...
                .addLore(playerMyServer.getModeratorCollection().stream()
                        .map(corePlayer -> "§8▸ " + corePlayer.getDisplayName()).toArray(value -> new String[playerMyServer.getModeratorCollection().size()]))

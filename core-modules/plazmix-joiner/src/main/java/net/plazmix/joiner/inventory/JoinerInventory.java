...
public class JoinerInventory extends CorePaginatedInventory {

    private final LocalizationResource localizationResource;

    public JoinerInventory(LocalizationResource localizationResource) {
    ...
    public void drawInventory(@NonNull CorePlayer player) {
        ...
        for (JoinMessageCategory category : JoinMessageCategory.values()) {
            addItem(category.getInventorySlot(), category.getItemStack(player), (player1, event) -> {

                new JoinerSelectMessageInventory(localizationResource, category).openInventory(player);
            });
        }

...
public class EconomyCoinsCommand extends VkCommand {
    ...
    protected void execute(BotUser botUser, @NonNull Message message, @NonNull String[] args, @NonNull VkBot vkBot) {
        if (args.length == 0) {
            vkBot.printMessage(message.getPeerId(), "❗ Ошибка в синтаксисе, используйте - !коины <ник> <кол-во>");
            return;
        }
        ...
        vkBot.printMessage(message.getPeerId(), "Игроку " + corePlayer.getName() + " было успешно добавлено " + Integer.parseInt(args[1]) + " коинов");
    }
